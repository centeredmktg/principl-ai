...
const castingProductionHtml = await Bun.file(
  new URL("./casting-production.html", import.meta.url).pathname
).text();
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (path === "/casting-production" && req.method === "GET") {
      return new Response(castingProductionHtml, {
        headers: { "Content-Type": "text/html; charset=utf-8" },
      });
    }
